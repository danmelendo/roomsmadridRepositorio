...
function RedsysReturnPage() {
  ...
  useEffect(() => {
    if (ko) {
      ...
      (async () => {
        const { data } = await supabase
          .from("reservations").select("id").eq("redsys_order", order).eq("deposit_paid", false).maybeSingle();
        if (data?.id) await supabase.from("reservations").update({ status: "rejected", cancellation_reason: "Pago no completado por el cliente" } as never).eq("id", data.id);
      })();
      return;
    }
    ...
    const check = async () => {
      const { data } = await supabase
        .from("reservations").select("deposit_paid, status").eq("redsys_order", order).maybeSingle();
      if (cancelled) return;
      if (data?.deposit_paid) { setState("confirmed"); return; }
  ...
  const ink = "#0b0c0c", soft = "#7a7066", blood = "#731423";

  return (
    <div style={{
      fontFamily: "'Noto Sans', sans-serif", background: "#f6f3ec", minHeight: "100svh",
      display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center",
      padding: "40px 24px", color: ink,
    }}>
      ...
      <img src="/brand/rooms-madrid-horizontal-negro.svg" alt="Rooms Madrid" style={{ height: 40, width: "auto", marginBottom: 28 }} />

      <div style={{
        background: "#fff", border: "1px solid rgba(115,20,35,0.18)", borderTop: `3px solid ${blood}`,
        borderRadius: 20, padding: "48px 40px", maxWidth: 460, width: "100%", textAlign: "center",
        boxShadow: "0 8px 48px rgba(11,12,12,0.08)",
      }}>
        {state === "checking" && (
          <>
            <Loader2 size={44} color={blood} style={{ marginBottom: 20, animation: "rmspin 1s linear infinite" }} />
            <h1 style={{ fontFamily: "'Playfair', serif", fontSize: 32, fontWeight: 600, marginBottom: 12 }}>
              Verificando tu pago…
            </h1>
        ...
        {state === "confirmed" && (
          <>
            <div style={{
              width: 72, height: 72, borderRadius: "50%", background: "rgba(115,20,35,0.10)",
              border: "1px solid rgba(115,20,35,0.28)", display: "flex", alignItems: "center",
              justifyContent: "center", margin: "0 auto 24px",
            }}>
              <CheckCircle2 size={32} color={blood} />
            </div>
            <h1 style={{ fontFamily: "'Playfair', serif", fontSize: 42, fontWeight: 600, marginBottom: 14 }}>
            ...
            {order && <p style={{ fontSize: 12, color: "#9a9082", marginTop: 4, letterSpacing: "0.06em" }}>Referencia de pago: {order}</p>}
          </>
        )}
        ...
        {state === "pending" && (
          <>
            ...
            <p style={{ fontSize: 15, color: soft, lineHeight: 1.65, marginBottom: 8 }}>
              No hemos podido confirmar el cobro todavía. Si se ha realizado, recibirás un email de confirmación en breve.
            </p>
            <p style={{ fontSize: 13, color: soft, lineHeight: 1.65, marginBottom: 24 }}>
              Si no recibes confirmación o tienes dudas, contáctanos antes de volver a intentarlo para no duplicar el cobro.
            </p>
          </>
        )}
        ...
        {state === "failed" && (
          <>
            ...
            <h1 style={{ fontFamily: "'Playfair', serif", fontSize: 36, fontWeight: 600, marginBottom: 12 }}>
              Pago no completado
            </h1>
            <p style={{ fontSize: 15, color: soft, lineHeight: 1.65, marginBottom: 8 }}>
        ...
        {state !== "checking" && (
          <Link to="/reservar" style={{
            ...
            background: "linear-gradient(180deg, #8b1027 0%, #731423 100%)", color: "#fff", border: "none",
            borderRadius: 10, height: 52, padding: "0 32px", fontSize: 13, fontWeight: 600,
            letterSpacing: "0.08em", textTransform: "uppercase", textDecoration: "none", marginTop: 8,
          }}>
            {state === "failed" ? "Intentar de nuevo" : state === "pending" ? "Volver" : "Hacer otra reserva"}
          </Link>
        )}
      </div>
